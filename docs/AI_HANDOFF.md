...
- `tools/ENSURE_NODE22.ps1` for verified portable Node 22 bootstrap when needed.

Local validation before pushing the debug tooling:

- probe syntax: pass;
- dedicated probe tests: **6/6 pass**;
- full Node test suite on the debug tree: **29/29 pass**;
...
3. let the branch runner prepare Node 22 if needed, validate the probe, then execute it;
