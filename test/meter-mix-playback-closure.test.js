...
test('focused mix campaign contains no output routing, direct protocol, raw, or mixer-slot source write path', () => {
	...
	for (const source of [closure, page]) {
...
test('focused mix launcher prepares Page 2 before permission and stays open for result capture', () => {
	...
	assert.ok(prep >= 0 && permission > prep)
