...
const RELEASE_REVISION = 'v1-release-smoke-v4-pairmeta-20260826'
...
async function captureSourceCatalog(baseUrl, label) {
	...
	for (let n = 1; n <= 128 && misses < 8; n++) {
		const [id, name, type, pairSide, pairId] = await Promise.all([
			...
			readVariableOptional(baseUrl, label, `source_${n}_pair_side`, 1800),
			readVariableOptional(baseUrl, label, `source_${n}_pair_root_id`, 1800),
		])
		...
		if (rawId && rawId !== '0') {
			routingSources.push({
				...
				pairSide: pairSide.exists ? String(pairSide.value || '').trim() : '',
				pairId: pairId.exists ? String(pairId.value || '').trim() : '',
			})
		}
	}
...
function schemaSourcePairs(sources) {
	const visible = directSourceCandidates(sources)
	const byId = new Map(visible.map((source) => [String(source.id), source]))
	const pairs = []
	for (const left of visible) {
		if (left.pairSide !== 'L' || !left.pairId) continue
		const right = byId.get(String(left.pairId))
		if (!right || right.pairSide !== 'R' || String(right.pairId || '') !== String(left.id)) continue
...
function addDirectPairRestoreTests(tests, values, routingSources) {
	...
	const sourcePairs = schemaSourcePairs(routingSources)
...
function buildReleaseTestsV4(values, catalog) {
...
async function executeTestV4(baseUrl, label, pageNumber, test, locations) {
...
async function main() {
	...
	console.log('')
	console.log('==================================================================')
	console.log(' FOCUSRITE 18i20 V1 RELEASE SMOKE V4 - EXACT SCHEMA PAIRS')
	console.log('==================================================================')
	...
	console.log('Output pair routing uses ONLY schema-confirmed reciprocal source pairs; names are never used as the pair oracle.')
	console.log('Every attempted write requires exact target restore plus a collateral-state audit.')
	console.log('No ALT/Stereo/Custom Mix/disruptive/raw/Monitor-gain write exists in this public-surface runner.')
	console.log('')
	...
	line('INFO', 'Schema-confirmed routing pairs', `${schemaPairs.length} reciprocal direct source pair(s) available to the smoke`)
	const tests = buildReleaseTestsV4(stable.values, catalog)
...
if (require.main === module) {
	main().catch((error) => {
		line('FAIL', 'V1 release smoke V4', error.message)
