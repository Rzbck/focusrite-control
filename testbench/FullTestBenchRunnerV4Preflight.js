...
function addStaticRows(inventory) {
  const add = (id, family, status, detail, risk = 'excluded') => inventory.rows.push({
    id, family, variable: '', availability: 'N/A', r9ProbeCount: 0, state: '', stateKnown: false,
    capability: false, risk, dependency: '', status, detail,
  })
...
async function prepareLab(reporter) {
  ...
  const inventory = buildCapabilityInventory({ model, shape, snapshot, coreInitial, r9Probes: r9.probes, availabilityMap })
  ...
  const counts = outputEligibility.reduce((acc, row) => { acc[row.availability] = (acc[row.availability] || 0) + 1; return acc }, {})
  line('INFO', 'Output capability', `AVAILABLE=${counts.AVAILABLE || 0} UNAVAILABLE=${counts.UNAVAILABLE || 0} UNKNOWN=${counts.UNKNOWN || 0} NO_FLAG=${counts.NO_FLAG || 0}`)
  ...
  if (!ext) {
    ...
    reporter.add('prepare', 'v4-harness', 'PREP_REQUIRED', `Generated isolated capability harness ${built.signature}; zero hardware writes.`)
    return { prep: 'harness', baseUrl, label, r9, safePlan, profile, shape, snapshot, coreInitial, inventory, outputEligibility, built, model }
  }
  ...
  return { prep: null, baseUrl, label, r9, safePlan, profile, shape, snapshot, coreInitial, inventory, outputEligibility, built, ext, model, revision: CAMPAIGN_REVISION }
}
